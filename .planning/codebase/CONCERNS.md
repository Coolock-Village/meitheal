...
### ~~Kanban User Filter Pills Show Full Name~~ (RESOLVED — Phase 06 Plan 01)
- Initials-only avatars with tooltip hover implemented

### ~~Kanban Board-Level tabindex Causes Drag Artifacts~~ (RESOLVED — Phase 06 Plan 01)
- Board-level `tabindex` removed, keyboard nav via card focus

### ~~Kanban Group-by-Board Shows Raw board_id~~ (RESOLVED — Phase 06 Plan 01)
- Board titles resolved from `/api/boards` with lookup map

### ~~Kanban Columns Have No Vertical Scroll~~ (RESOLVED — Phase 06 Plan 01)
- `max-height: calc(100vh - 280px)` + `overflow-y: auto` added to `.kanban-cards`

### ~~Board Filter Not Integrated with Kanban Toolbar~~ (RESOLVED — Phase 06 Plans 01+07)
- Board filter toolbar added, swimlane counts updated via unified `updateAllCounts()`
