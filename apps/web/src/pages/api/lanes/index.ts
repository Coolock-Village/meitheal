...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!key) {
    return new Response(JSON.stringify({ error: "Invalid lane key" }), {
      status: 400,
      headers: { "content-type": "application/json" },
    });
  }
  ...
  if (existing.rows.length > 0) {
    return new Response(JSON.stringify({ error: "Lane with this key already exists" }), {
      status: 409,
      headers: { "content-type": "application/json" },
    });
  }
  ...
  return new Response(JSON.stringify({ id, key, label, icon, position, wip_limit: wipLimit, includes: [key], builtIn: false, created_at: now, updated_at: now }), {
    status: 201,
    headers: { "content-type": "application/json" },
  });
};
